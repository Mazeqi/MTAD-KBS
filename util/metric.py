...
class Evaluator(object):
    def __init__(self, metrics=[], pooling_ks=None, max_step_aupro=200, mp=False, use_adeval=False):
        ...
        self.roc_data = {}
    def plot_all_rocs(self, save_path='visa_rocs.eps'):
        """紧凑图例布局（图例嵌入绘图区）"""
        # 期刊格式参数
        plt.rcParams.update({
            'font.family': 'serif',
            ...
            'ytick.labelsize': 7,
            'lines.linewidth': 1.2,
            'figure.figsize': (4, 4),  # 宽高一致的正方形
            'legend.framealpha': 0.9
        })
        
        # 创建画布（增加高度以容纳图例）
        fig = plt.figure(figsize=(3.3, 2.3))  # 宽度3.3英寸，高度4.5英寸
        ...
        # 绘制曲线（缩短标签文本）
        for idx, (cls_name, (fpr, tpr, roc_auc)) in enumerate(self.roc_data.items()):
            plt.plot(fpr, tpr, 
                    color=colors[idx], 
                    linestyle=line_styles[idx % 1],
                    label=f'{cls_name[:]} ({roc_auc:.2f})',  # 截断至8字符
                    alpha=0.9,
                    rasterized=False)
        
        # 基准线与美化
        plt.plot([0, 1], [0, 1], 'k--', linewidth=0.5, dashes=(2, 1))
        plt.xlim(0, 1)
        plt.ylim(0.0, 1.02)
        plt.xlabel('False Positive Rate (FPR)')
        plt.ylabel('True Positive Rate (TPR)')
        plt.grid(True, linestyle=':', alpha=0.2, linewidth=0.3)
        
        # 调整绘图区边距（为图例腾出空间）
        plt.subplots_adjust(right=0.65)  # 右侧留出35%空间
        
        # 紧凑多列图例（嵌入右上角）
        leg = plt.legend(
            loc='lower center',           # 定位锚点
            bbox_to_anchor=(0.75, 0.05),  # 锚点紧贴右下角
            ncol=1,                       # 两列布局
            columnspacing=1.2,            # 列间距
            handletextpad=0.5,            # 文本与图例句柄间距
            frameon=False,
            borderaxespad=0.5             # 图例与轴间距
        )
    ...
    def run(self, results, cls_name, logger=None):
        ...
        for metric in self.metrics:
            ...
            if metric.startswith('mAUROC_sp_max'):
                ...
                # fpr, tpr, roc_thresholds = roc_curve(gt_sp, pr_sp_max)
                # roc_auc = auc(fpr, tpr)

                # self.roc_data[cls_name] = (fpr, tpr, roc_auc)
