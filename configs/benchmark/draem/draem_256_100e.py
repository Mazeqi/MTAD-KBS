...
class cfg(cfg_common, cfg_dataset_default, cfg_model_draem):
	...
	def __init__(self):
		...
		self.lr = 1e-4 * self.batch_train / 16
		self.metrics = [
		...
		# ==> evaluator
		self.evaluator.kwargs = dict(metrics=self.metrics, pooling_ks=[21, 21], max_step_aupro=100, use_adeval=self.use_adeval)
		...
		# ==> trainer
		self.trainer.name = 'DRAEMTrainer'
		...
		self.loss.loss_terms = [
			dict(type='L2Loss', name='mse', lam=1.0),
			dict(type='SSIMLoss', name='ssim', lam=1.0),
			dict(type='FocalLoss', name='focal', lam=1.0),
		]
